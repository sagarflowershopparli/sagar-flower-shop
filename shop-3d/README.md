...
With Python Playwright installed and Chrome available at `/opt/google/chrome/chrome`, run `python3 shop-3d/scripts/smoke-test.py`. It starts a temporary local server and writes screenshots to a temporary directory. The check covers lazy entry, desktop hover/selection, product dialog and WhatsApp destination, all guided stops, mobile tap/layout, catalog return, quality mode, wheel/keyboard navigation, normal-motion automatic entry and touch swipe, WebGL context loss, Save-Data, required-asset failure, and the homepage invitation. It does not send WhatsApp messages.
